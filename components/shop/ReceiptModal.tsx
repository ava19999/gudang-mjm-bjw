...
export const ReceiptModal: React.FC<ReceiptModalProps> = ({ 
    ...
    const { getStoreConfig } = useStore();
    const storeConfig = getStoreConfig();
    const receiptTitleName = storeConfig?.name ?? storeConfig?.fullName ?? 'Store';
    const receiptHeaderName = storeConfig?.fullName ?? storeConfig?.name ?? 'Store';
    ...
    const handlePrint = () => {
        ...
        printWindow.document.write(`
            ...
            <html>
            <head>
                <title>${receiptTitleName} - Resi ${customerName}</title>
                <style>
                    body {
    ...
    const handleShare = async () => {
        ...
        const file = new File([blob], `${receiptTitleName}-${customerName}.jpg`, { type: 'image/jpeg' });

        if (navigator.share && navigator.canShare?.({ files: [file] })) {
            try {
                await navigator.share({
                    files: [file],
                    title: `${receiptTitleName} - Resi Pesanan`,
                    text: `Resi pesanan ${receiptTitleName} untuk ${customerName}`,
                });
            } catch (error) {
            ...
            const whatsappText = encodeURIComponent(`${receiptTitleName} - Resi Pesanan\nCustomer: ${customerName}\nTotal: ${formatRupiah(cartTotal)}`);
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
            ...
            <div className="bg-gray-800 rounded-3xl shadow-2xl w-full max-w-md relative overflow-hidden animate-in zoom-in-95 border border-gray-700 max-h-[90vh] overflow-y-auto">
                <div className="bg-gray-900 px-6 py-4 border-b border-gray-700 flex justify-between items-center sticky top-0 z-10">
                    <h3 className="text-lg font-bold text-gray-100">{receiptTitleName} - Resi Pesanan</h3>
                ...
                <div ref={receiptRef} className="bg-white p-6">
                    ...
                    <div className="text-center mb-4 border-b-2 border-gray-800 pb-3">
                        <h1 className="text-xl font-bold text-gray-900 mb-0.5">{receiptHeaderName}</h1>
