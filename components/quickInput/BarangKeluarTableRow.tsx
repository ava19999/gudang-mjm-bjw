// FILE: src/components/quickInput/BarangKeluarTableRow.tsx
import React, { useEffect, useRef, useState } from 'react';
import { BarangKeluarRow } from './types';
import { InventoryItem } from '../../types';
import { checkIsBarangKeluarRowComplete } from './quickInputUtils';
import { formatRupiah } from '../../utils';
import { Loader2, AlertCircle, Check, Trash2, Info } from 'lucide-react';

interface BarangKeluarTableRowProps {
    row: BarangKeluarRow;
    index: number;
    globalIndex: number;
...
export const BarangKeluarTableRow: React.FC<BarangKeluarTableRowProps> = ({
    ...
    const isComplete = checkIsBarangKeluarRowComplete(row);
    const activeItemRef = useRef<HTMLDivElement>(null);
    const [showItemDetails, setShowItemDetails] = useState(false);

    // Constants - 10 input columns for Barang Keluar
    const COLS = 10;
    const baseRefIndex = globalIndex * COLS;

    // Payment terms options - includes NADIR for Barang Keluar
    ...
    const handleQtyKeluarChange = (valStr: string) => {
        ...
        const currentTotal = row.totalHargaKeluar || 0;
    ...
    // When Total Harga Keluar changes -> recalculate Harga Satuan
    const handleTotalHargaKeluarChange = (valStr: string) => {
        const cleanVal = valStr.replace(/[^0-9]/g, '');
        ...
        if (row.qtyKeluar > 0) {
            ...
            onUpdateRow(row.id, {
                totalHargaKeluar: newTotal,
                hargaSatuan: newUnitPrice
            });
        } else {
            // If qtyKeluar is 0, just update totalHargaKeluar
            onUpdateRow(row.id, {
                totalHargaKeluar: newTotal,
                hargaSatuan: 0
            });
    ...
    return (
        <>
            <tr className={`hover:bg-gray-700/20 border-b border-gray-700/50 ${row.error ? 'bg-red-900/10' : ''}`}>
                ...
                {/* COL 4: Barang (read-only, filled from selection) */}
                <td className="px-2 py-1.5">
                    <div className="px-1 py-1 text-xs text-gray-300 truncate" title={row.namaBarang}>
                        {row.namaBarang || '-'}
                    </div>
                </td>

                {/* COL 5: Brand (read-only, filled from selection) */}
                <td className="px-2 py-1.5">
                    <div className="px-1 py-1 text-xs text-gray-400 truncate" title={row.brand}>
                        {row.brand || '-'}
                    </div>
                </td>

                {/* COL 6: Aplikasi (read-only, filled from selection) */}
                <td className="px-2 py-1.5">
                    <div className="px-1 py-1 text-xs text-gray-400 truncate" title={row.aplikasi}>
                        {row.aplikasi || '-'}
                    </div>
                </td>

                {/* COL 7: Rak (read-only, filled from selection) */}
                <td className="px-2 py-1.5">
                    <div className="px-1 py-1 text-xs text-gray-400 truncate" title={row.rak}>
                        {row.rak || '-'}
                    </div>
                </td>

                {/* COL 8: Qty Saat Ini (read-only, filled from selection) */}
                <td className="px-2 py-1.5">
                    <div className="px-1 py-1 text-xs font-mono text-right text-blue-300">
                        {row.qtySaatIni || 0}
                    </div>
                </td>
                ...
                <td className="px-2 py-1.5">
                    <input
                        ...
                        type="text"
                        inputMode="numeric"
                        className="w-full bg-transparent px-1 py-1 text-xs font-bold text-right font-mono text-red-400 focus:outline-none focus:text-red-300 placeholder-gray-600"
                        value={row.qtyKeluar || ''}
                ...
                <td className="px-2 py-1.5">
                    <input
                        ...
                        value={row.totalHargaKeluar || ''}
                        onChange={(e) => handleTotalHargaKeluarChange(e.target.value)}
                        onKeyDown={(e) => onGridKeyDown(e, baseRefIndex + 5)}
                        placeholder="Total..."
                    />
                </td>

                {/* COL 11: Total Harga Satuan (Read-only, calculated) */}
                <td className="px-2 py-1.5">
                    <div className="px-1 py-1 text-xs font-mono text-right text-green-300 opacity-70" title="Auto-calculated: Total / Qty">
                        ...
                        <div className="text-xs text-gray-300 grid grid-cols-4 gap-2">
                            ...
                            {row.qtySaatIni !== undefined && (
                                <div>
                                    ...
                                    <div className="font-medium text-blue-400">{row.qtySaatIni}</div>
                                </div>
                            )}
                        </div>
                    </td>
